    ...
    hint(game, player) {
        ...
        for (let i = 0; i < moveCounter.length; i++) {
            if (moveCounter[i].playerID === player) {
                ...
                let maxMoveIndex = moveCounts.findIndex(t => t === maxMoves);
                for (let k = 0; k < WinRules[maxMoveIndex].length; k++) {
                    for (let h = 0; h < playerMoves.length; h++) {
                        if (playerMoves[h].playerID === player &&
                            playerMoves[h].move.positionX != WinRules[maxMoveIndex][k].positionX &&
                            playerMoves[h].move.positionY != WinRules[maxMoveIndex][k].positionY) {
                            if (this.checkNearby(playerMoves[h].move, WinRules[maxMoveIndex][k])) {
                                let checkExist = false;
                                checkExist = playerMoves.findIndex(t => WinRules[maxMoveIndex][k].positionX == t.positionX &&
                                    WinRules[maxMoveIndex][k].positionY == t.positionY) >= 0;
                                if (!checkExist &&
                                    hintMove.findIndex(t => t.positionX == WinRules[maxMoveIndex][k].positionX &&
                                        t.positionY == WinRules[maxMoveIndex][k].positionY) < 0) {
                                    hintMove.push(WinRules[maxMoveIndex][k]);
                                    break;
                                }
                            }
                        }
